...
function createLocation(current, to, state = null, key) {
  let location = {
    pathname: true ? current : current.pathname,
    search: "",
    hash: "",
...
function unflatten(parsed) {
  const { hydrated, values: values2 } = this;
  if (typeof parsed === "number") return hydrate.call(this, parsed);
  if (!Array.isArray(parsed) || !parsed.length) throw new SyntaxError();
  const startIndex = values2.length;
  ...
  while (stack.length > 0) {
    ...
    if (Array.isArray(value)) {
      if (typeof value[0] === "string") {
        ...
        switch (type) {
          case TYPE_DATE:
            set(hydrated[index22] = new Date(b2));
            continue;
          case TYPE_URL:
            set(hydrated[index22] = new URL(b2));
            continue;
          case TYPE_BIGINT:
            set(hydrated[index22] = BigInt(b2));
            continue;
          case TYPE_REGEXP:
            set(hydrated[index22] = new RegExp(b2, c));
            continue;
          case TYPE_SYMBOL:
            set(hydrated[index22] = Symbol.for(b2));
            continue;
          case TYPE_SET:
          ...
          case TYPE_PROMISE:
            if (hydrated[b2]) {
              set(hydrated[index22] = hydrated[b2]);
            } else {
...
`));
        ...
        controller.enqueue(
          textEncoder2.encode(`[${encoder2.stringified.join(",")}]
`)
        );
...
`
                    ...
                    const values2 = encoder2.stringified.slice(lastSentIndex + 1).join(",");
                    controller.enqueue(
                      textEncoder2.encode(
                        `${TYPE_PROMISE}${deferredId}:[${values2}]
`
                      )
                    );
                    lastSentIndex = encoder2.stringified.length - 1;
...
function Connection(options, queues = {}, { onopen = noop$1, onend = noop$1, onclose = noop$1 } = {}) {
  ...
  async function cancel({ pid: pid2, secret }, resolve, reject) {
    try {
      cancelMessage = b().i32(16).i32(80877102).i32(pid2).i32(secret).end(16);
      await connect();
...
function parseOptions(a, b2) {
  if (a && a.shared)
    return a;
  const env2 = process$1.env, o = (!a || typeof a === "string" ? b2 : a) || {}, { url, multihost } = parseUrl(a), query = [...url.searchParams].reduce((a2, [b3, c]) => (a2[b3] = c, a2), {}), host = o.hostname || o.host || multihost || url.hostname || env2.PGHOST || "localhost", port = o.port || url.port || env2.PGPORT || 5432, user = o.user || o.username || url.username || env2.PGUSERNAME || env2.PGUSER || osUsername();
...
function osUsername() {
  try {
    ...
    return process$1.env.USERNAME || process$1.env.USER || process$1.env.LOGNAME;
  }
}
const mockBooks = [
  {
...
class BooksService extends WorkerEntrypoint {
  ...
  async getBook(bookId) {
    ...
    try {
      if (dbAvailable && sql) {
        ...
        const book = mockBooks.find((book2) => book2.id === bookIdNum);
        if (!book) {
  ...
  async getRelatedBooks(bookId) {
    ...
    try {
      if (dbAvailable && sql) {
        ...
        const relatedBooks = mockBooks.filter((b2) => b2.genre === bookGenre && b2.id !== bookIdNum).slice(0, 3);
        const recentBooks = mockBooks.filter((b2) => b2.id !== bookIdNum).slice(0, 2);
        const genres = {};
        mockBooks.forEach((b2) => {
          genres[b2.genre] = (genres[b2.genre] || 0) + 1;
        });
    ...
    try {
      if (dbAvailable && sql) {
        ...
        if (sort) {
          switch (sort) {
            case "title_asc":
              query = genre ? sql`SELECT * FROM public.books WHERE genre = ${genre} ORDER BY title ASC` : sql`SELECT * FROM public.books ORDER BY title ASC`;
              break;
            case "title_desc":
              query = genre ? sql`SELECT * FROM public.books WHERE genre = ${genre} ORDER BY title DESC` : sql`SELECT * FROM public.books ORDER BY title DESC`;
              break;
            case "author_asc":
              query = genre ? sql`SELECT * FROM public.books WHERE genre = ${genre} ORDER BY author ASC` : sql`SELECT * FROM public.books ORDER BY author ASC`;
              break;
            case "author_desc":
              query = genre ? sql`SELECT * FROM public.books WHERE genre = ${genre} ORDER BY author DESC` : sql`SELECT * FROM public.books ORDER BY author DESC`;
              break;
            default:
              break;
          }
        ...
        if (sort) {
          switch (sort) {
            case "title_asc":
              results.sort((a, b2) => a.title.localeCompare(b2.title));
              break;
            case "title_desc":
              results.sort((a, b2) => b2.title.localeCompare(a.title));
              break;
            case "author_asc":
              results.sort((a, b2) => a.author.localeCompare(b2.author));
              break;
            case "author_desc":
              results.sort((a, b2) => b2.author.localeCompare(a.author));
              break;
            default:
              break;
          }
...
const requestHandler = createRequestHandler(
  () => import("./server-build-DyFFcaJb.js"),
  "production"
);
app.get("*", async (c) => {
...
export {
  ...
  Outlet as O,
  ServerRouter as S,
  withHydrateFallbackProps as a,
  withComponentProps as b,
